# Vendored from Sendspin/sendspin-python-cli (sendspin/audio.py), Apache-2.0.
# Copyright contributors of the Sendspin project. Adapted for LVA:
#   - package-local imports; numpy-only volume path (no C extension)
#   - duck-gain hook for voice-coordination (LVA EventBus integration)
#   - PCM-only (LVA advertises PCM, as the spec requires servers to support)
"""Synchronized audio output stage for the Sendspin client."""
...
class AudioPlayer:
    ...
    def __init__(
        ...
        # LVA: voice-coordination ducking (multiplier applied on top of volume)
    ...
    def set_duck_gain(self, gain: float) -> None:
        """LVA: set the voice-coordination duck multiplier (0.0-1.0).

        Applied on top of the volume curve; 1.0 = no ducking.
        """
    ...
    def _apply_volume(self, output_buffer: memoryview) -> None:
        ...
        if muted or volume == 0 or duck_gain == 0.0:
            # Fill with silence
            self._fill_silence(output_buffer, 0, len(output_buffer))
            return

        if volume == 100 and duck_gain == 1.0:
            return
